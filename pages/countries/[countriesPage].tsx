...
import { Link, Card, CardContent, CardMedia, Container, Grid, Typography, Button, IconButton } from "@mui/material";
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const countriesSlugs = countries.map((country: any) => country.cca3.toLowerCase());
  ...
  export const getStaticProps: GetStaticProps<HomePageProps>=async({params})=>{
    ...
    const response = await fetch (`https://restcountries.com/v3.1/alpha/${countrySelect}`);
        ...
        <>
          {countriesData.map((country) => (
            ...
            [theme.breakpoints.down('lg')]: {
              ...
              <Grid item xs={12} md={6} lg={6} xl={12} sx={{
                p:"0px 0px 20px 0px !important"
              }}>
                  <Link href="/">
                 <IconButton sx={{
                  backgroundColor: "#fff",
                  borderRadius: "10px",
                  fontSize:"18px"
                 }}><ArrowBackIcon sx={{pr:"10px",fontSize:"30px"}}/> Go Back Home</IconButton> 
                  </Link>
              </Grid>
